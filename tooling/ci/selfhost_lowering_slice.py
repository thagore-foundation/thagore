...
def main() -> int:
    ...
    parser.add_argument("--lower-bin", required=True)
    parser.add_argument("--manifest", default="bootstrap/selfhost/corpus/lowering-slice.txt")
    parser.add_argument("--report-out", default="")
    args = parser.parse_args()

    repo_root = pathlib.Path(args.repo_root).resolve()
    lower_bin = pathlib.Path(args.lower_bin).resolve()
    report_lines: list[str] = []

    for fixture, expected_path in load_manifest(repo_root / args.manifest):
        completed = subprocess.run(
            [str(lower_bin), str((repo_root / fixture).resolve().as_posix())],
            cwd=repo_root,
            check=False,
            capture_output=True,
            text=True,
    ...
    print("selfhost lowering slice ok")
    return 0
