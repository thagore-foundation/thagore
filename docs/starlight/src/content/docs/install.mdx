---
title: Install
description: Build the current Thagore toolchain from source and verify that thagc, thagore-fmt, thagore-lsp, and drago work on your machine.
sidebar:
  order: 0
---
...
The current repository builds the Thagore toolchain from source. This page documents the workflow that matches the codebase as it exists today: `thagc`, `thagore-fmt`, and `thagore-lsp` are Rust binaries built with Cargo, and `drago` is a Thagore program bootstrapped by `thagc`.

## What you will build

The source tree currently provides these command-line tools:
...
| `drago` | Package manager, built with `thagc` | separate repo |
...
## Optional: install binaries into your PATH
