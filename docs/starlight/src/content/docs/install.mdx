...
```bash
curl -fsSL https://github.com/thagore-foundation/thagore/releases/download/v0.9.1/thagup.sh | sh -s -- --prefix "$HOME/.thagore"
```

### Install a specific target or channel

```bash
curl -fsSL https://github.com/thagore-foundation/thagore/releases/download/v0.9.1/thagup.sh | sh -s -- \
  --channel extended \
