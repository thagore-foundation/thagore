...
```bash
curl -fsSL https://github.com/thagore-foundation/thagore/releases/download/v0.9.2/thagup-v0.9.2.sh | sh -s -- --prefix "$HOME/.thagore"
```

### Install a specific target or channel

```bash
curl -fsSL https://github.com/thagore-foundation/thagore/releases/download/v0.9.2/thagup-v0.9.2.sh | sh -s -- \
  --channel extended \
...
The current `drago` codebase expects `thagc >= 0.9.2`.
