---
title: std.time
description: Time utilities in the Thagore standard library.
sidebar:
  order: 11
---

# std.time

The `std.time` module provides access to the current runtime clock and basic sleep control.

## Import

```thagore
import std.time as time
```

## API

| Function | Signature | Description |
|----------|-----------|-------------|
| `now_ms` | `func now_ms() -> i64` | Current time in milliseconds since epoch |
| `monotonic_ms` | `func monotonic_ms() -> i64` | Monotonic clock in milliseconds (not subject to wallclock jumps) |
| `sleep_ms` | `func sleep_ms(ms: i64)` | Pause the current thread for at least `ms` milliseconds |

## Example

```thagore
import std.time as time

func main() -> i32:
  let t = time.now_ms()
  return 0
```

`now_ms` returns `i64`. There is no `string.from_i64` in the current standard library, so printing a timestamp directly is not supported without an intermediate cast. To compare timestamps, use arithmetic:

## Timing elapsed duration

```thagore
import std.time as time

func main() -> i32:
  let start = time.now_ms()
  let i: i32 = 0
  while (i < 1000000):
    i = i + 1
  let elapsed = time.now_ms() - start
  if (elapsed > 0):
    return 0
  return 1
```

## Sleeping

```thagore
import std.time as time

func main() -> i32:
  let start = time.now_ms()
  time.sleep_ms(25)
  let elapsed = time.now_ms() - start
  if (elapsed >= 10):
    return 0
  return 1
```

## Full reference

| Function | Signature | Description |
|----------|-----------|-------------|
| `now_ms` | `func now_ms() -> i64` | milliseconds since epoch |
| `monotonic_ms` | `func monotonic_ms() -> i64` | monotonic clock milliseconds, unaffected by wallclock changes |
| `sleep_ms` | `func sleep_ms(ms: i64)` | sleep for at least `ms` milliseconds |
