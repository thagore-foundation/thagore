...
Downloads the latest **official public release** for your platform and updates the managed toolchain at `~/.thagore/toolchains/stable` (or `%USERPROFILE%\.thagore\toolchains\stable` on Windows).
...
`update apply` updates the managed toolchain (`bin` + bundled shared libraries) and version metadata. Your source files and project configurations are not touched.
</Aside>
...
Rollback restores the **immediately previous** managed toolchain version only. Running `update apply` twice without a rollback in between will overwrite the backup.
</Aside>
