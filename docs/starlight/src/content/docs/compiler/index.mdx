...
The Thagore compiler is written in Rust and targets self-hosting. As of v0.9.7, the compiler can rebuild itself (`thagc` compiled by `thagc`) with a verified fixed-point hash on Linux and Windows. macOS self-hosting is gated in CI and pending a macOS runner lane.
