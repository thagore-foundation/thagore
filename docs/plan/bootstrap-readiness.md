...
| --- | --- | --- | --- | --- |
| bootstrap probe | in progress | yes | ducknogit | probe exists under `tests/bootstrap_probe/`, imports multiple local modules, formats diagnostic-like report lines, gates deterministic output on Linux x64 and Windows x64, and now passes a public-toolchain rehearsal fed by `releases/latest` on both required lanes; the first self-written bootstrap seed now also lives under `tests/bootstrap_seed/`, emits an escape-free token stream, and is gated on both Linux x64 and Windows x64 |
