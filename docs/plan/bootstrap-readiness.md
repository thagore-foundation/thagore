...
| --- | --- | --- | --- | --- |
| bootstrap probe | in progress | yes | ducknogit | probe exists under `tests/bootstrap_probe/`, imports multiple local modules, formats diagnostic-like report lines, gates deterministic output on Linux x64 and Windows x64, and now passes a public-toolchain rehearsal fed by `releases/latest` on both required lanes; the first self-written bootstrap seed now also lives under `tests/bootstrap_seed/`, emits an escape-free token stream plus parser-lite/symbol-table-lite/diagnostics-lite output, resolves flat and dotted imports from fixture modules, and now gates missing-main, unknown-callee, unknown-import-alias-usage, call-arity-mismatch, primitive and simple-call return/local/assignment type mismatch, assignment-to-unknown-local, condition-type-mismatch, duplicate-import, duplicate-import-alias, duplicate-imported-symbol, local/func-shadowing-over-imports and aliases including later locals, duplicate-func, and unknown-imported-symbol semantic-lite checks on both Linux x64 and Windows x64 |
