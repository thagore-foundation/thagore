...
NAIVE_SRC = ROOT / "examples" / "intent_real_lowerbound_naive.tg"
INTENT_SRC = ROOT / "examples" / "intent_real_lowerbound_intent.tg"
...
def detect_compiler() -> Path:
    ...
    for c in candidates:
        if c.exists():
            return c
    raise SystemExit("FAIL: compiler not found (expected stage2(.exe) or stage1(.exe)).")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark lower_bound intent rewrite")
...
def main() -> int:
    ...
    naive_exe = ROOT / "intent_real_lowerbound_naive.exe"
    intent_exe = ROOT / "intent_real_lowerbound_intent.exe"
    ...
    print("=== LowerBound Intent Benchmark ===")
    print("unit: seconds")
    print("")
