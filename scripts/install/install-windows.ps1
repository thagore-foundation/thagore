...
if ((Test-Path (Join-Path $candidatePackageRoot "bin\thagc.exe")) -and (Test-Path (Join-Path $candidatePackageRoot "lib\std"))) {
    $RootDir = (Resolve-Path $candidatePackageRoot).Path
} elseif ((Test-Path (Join-Path $candidatePackageRoot "bin\thagore.exe")) -and (Test-Path (Join-Path $candidatePackageRoot "lib\std"))) {
    $RootDir = (Resolve-Path $candidatePackageRoot).Path
} elseif (Test-Path (Join-Path $candidateRepoRoot "dist\bin\thagc.exe")) {
...
if (-not (Test-Path $thagcBin) -and (Test-Path $thagoreBin)) {
    Copy-Item -Force $thagoreBin $thagcBin
}
if (-not (Test-Path $thagoreBin) -and (Test-Path $thagcBin)) {
    Copy-Item -Force $thagcBin $thagoreBin
}
...
function Test-InstalledAtomicBundle {
    ...
    $tmpDir = Join-Path $env:TEMP ("thagore-install-atomic-smoke-" + [guid]::NewGuid().ToString("N"))
...
'@ | Set-Content -Path $src -NoNewline
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Installed atomic smoke failed: build command failed."
        }
        if (-not (Test-Path $bin)) {
            throw "Installed atomic smoke failed: build output missing."
        }
        $runOut = (& $bin 2>&1 | Out-String)
        if ($runOut -notmatch "atomic package smoke") {
            throw "Installed atomic smoke failed: runtime output mismatch."
        }
    } finally {
...
Test-InstalledAtomicBundle -CompilerExe $thagcBin -RuntimeLib (Join-Path $Prefix "lib\runtime.lib")

Write-Host "Thagore installed successfully."
Write-Host "Binary: $thagcBin"
